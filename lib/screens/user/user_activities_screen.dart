...
class _UserActivitiesScreenState extends State<UserActivitiesScreen> {
  ...
  Future<void> _loadUserData() async {
    ...
    if (mounted) {
      ...
      if (user != null) {
        await _loadAllActivities(user.id);
      }
    }
  }
  ...
  Future<void> _loadAllActivities(int userId) async {
    ...
    try {
      ...
      final response = await api.ApiService().getRecentActivities(
        userId: userId,
        userType: 'users',
      );
          ...
          IconButton(
            ...
            onPressed: () {
              if (_currentUser != null) {
                _loadAllActivities(_currentUser!.id);
              }
            },
          ),
        ],
  ...
  Widget _buildContent(Color primaryBlue) {
          ...
          children: [
            ...
            Text(
              _searchQuery.isNotEmpty 
                  ...
                  : 'Your activities will appear here when available',
              style: TextStyle(
                fontSize: 13,
                color: Colors.grey[500],
              ),
    ...
    return RefreshIndicator(
      onRefresh: () async {
        if (_currentUser != null) {
          await _loadAllActivities(_currentUser!.id);
        }
      },
